...
"""
AmsiScanBufferPrototype = gdef.WINFUNCTYPE(gdef.BOOL, gdef.PVOID, gdef.PVOID, gdef.ULONG, gdef.LPCWSTR, gdef.PVOID,
                                           gdef.POINTER(gdef.ULONG))
AmsiScanBufferParams = (
(1, 'amsiContext'), (1, 'buffer'), (1, 'length'), (1, 'contentName'), (1, 'session'), (1, 'result'))
...
class _AMSI_RESULT(gdef.EnumType):
    values = [AMSI_RESULT_CLEAN, AMSI_RESULT_NOT_DETECTED, AMSI_RESULT_BLOCKED_BY_ADMIN_START,
              AMSI_RESULT_BLOCKED_BY_ADMIN_END, AMSI_RESULT_DETECTED]
...
class AMSIScanner(object):
    def __enter__(self):
        ...
        a = AmsiInitialize("TestEngine", self.context)
