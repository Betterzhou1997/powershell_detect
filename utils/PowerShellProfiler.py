...
def scoreBehaviors(behaviorTags):
    ...
    for behavior in behaviorTags:
        ...
        if "Known Malware:" in behavior:
            behaviorData.append(
                "%s: %s - %s" % (behavior.split(":")[0], behavior.split(":")[1], scoreValues[behavior.split(":")[0]]))
...
def family_finder(contentData):
    ...
    if all(entry in contentData.lower() for entry in
           ["buffer", "getstring", "getbytes", "networkinformation.ping", "dontfragment"]) or \
    ...
    if any(entry in contentData.lower() for entry in [
        ...
        "get-posthashdumpscript", ]) or \
            ...
            re.search("\$RegPath = .+\$parts = \$RegPath\.split.+\$path = \$RegPath\.split", contentData,
                      re.IGNORECASE) or \
    ...
    if all(entry in contentData.lower() for entry in ["dumpcred", "dumpcert", "customcommand"]) or \
            all(entry in contentData.lower() for entry in
                ["virtualprotect.invoke", "memcpy.invoke", "getcommandlineaaddr"]) or \
            all(entry in contentData.lower() for entry in
                ["[parameter(parametersetname = ", ", position = 1)]", "[switch]", "autolayout", "ansiclass"]) or \
    ...
    if all(entry in contentData.lower() for entry in ["function get-timedscreenshot", "#load required assembly"]) or \
            any(entry in contentData.lower() for entry in
    ...
    if any(entry in contentData.lower() for entry in ["$doit = @"]) or \
            all(entry in contentData.lower() for entry in
                ["func_get_proc_address", "func_get_delegate_type", "getdelegateforfunctionpointer", "start-job"]):
    ...
    if all(entry in contentData.lower() for entry in
           ["function get-key", "importdll", "check for keys not mapped by virtual keyboard"]) or \
...
def typeConversion(contentData, modificationFlag):
    ...
    for baseValue in [0, 8, 16, 32]:
        ...
        for entry in re.findall(r"([1-2]?[0-9][0-9](?:\s*),|0x[0-9a-fA-F]{1,2}(?:\s*),|\\x[0-9a-fA-F]{1,2}(?:\s*),)",
                                contentData.replace(" ", "")):
...
def normalize(contentData):
    ...
    while True:
        ...
        if re.search(r"([1-2]?[0-9]?[0-9](?:\s*),|0x[0-9a-fA-F]{1,2}(?:\s*),|\\x[0-9a-fA-F]{1,2}(?:\s*),)",
                     contentData):
...
def main():
    parser = argparse.ArgumentParser(
        description="PowerShellProfiler analyzes PowerShell scripts statically to identify and score behaviors.")
    parser.add_argument("-f", "--file", help="PowerShell Script to behaviorally profile", metavar="<file_name>",
                        required=True)
    ...
    startTime = datetime.now()
    alternativeData = "\n\n##### ORIGINAL SCRIPT #####\n\n%s\n\n##### ALTERED SCRIPT #####\n\n%s" % (
    originalData, unravelContent(originalData))
    stopTime = datetime.now() - startTime
    if debugFlag:
